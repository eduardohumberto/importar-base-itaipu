...
class Category {
    ...
    public function create_category_term( $name, $parent = 0 ){
        ...
        if( isset( $category->error_data )){
            add_term_meta( $category->error_data['term_exists'], 'socialdb_category_owner', '1');
            add_term_meta( $category->error_data['term_exists'], IDENTIFIER_IMPORT, IDENTIFIER_IMPORT);
            return $category->error_data['term_exists'];
        } else if( isset( $category['term_id'] ) ):
            add_term_meta( $category['term_id'], 'socialdb_category_owner', '1');
            add_term_meta( $category['term_id'], IDENTIFIER_IMPORT, IDENTIFIER_IMPORT);
